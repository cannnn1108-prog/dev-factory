...
function AccountSwitcher() {
  ...
  if (profiles.length <= 1) {
    const name = currentProfile?.x_username || currentProfile?.display_name || "アカウント";
    return (
  ...
  return (
    <div className="relative">
      <button
        ...
        className="w-full flex items-center gap-2 px-3 py-2 rounded-xl bg-dark-700/50 hover:bg-dark-600/50 transition-all"
      >
        <div className="w-6 h-6 rounded-full bg-gradient-to-br from-neon-blue to-neon-purple flex items-center justify-center">
          <svg className="w-3 h-3 text-white" viewBox="0 0 24 24" fill="currentColor">
            <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
        ...
        <span className="text-xs text-gray-300 truncate flex-1 text-left">
          {currentProfile?.x_username || currentProfile?.display_name || "アカウント"}
        </span>
        <ChevronDown className={`w-3 h-3 text-gray-500 transition-transform ${open ? "rotate-180" : ""}`} />
      </button>

      {open && (
        <div className="absolute bottom-full left-0 right-0 mb-1 bg-dark-800 border border-neon-indigo/20 rounded-xl overflow-hidden shadow-lg z-50">
          <div className="px-3 py-2 border-b border-neon-indigo/10">
            <span className="text-[10px] text-gray-500 uppercase tracking-wider">アカウント切替</span>
          </div>
          {profiles.map((profile) => (
            <button
              ...
              className={`w-full flex items-center gap-2 px-3 py-2.5 text-left hover:bg-dark-600/50 transition-all ${
                profile.id === currentProfile?.id ? "bg-neon-indigo/10" : ""
              }`}
            >
              <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${
                ...
                {(profile.x_username || profile.display_name || "?")[0]}
              </div>
              <span className={`text-xs truncate flex-1 ${
                profile.id === currentProfile?.id ? "text-white" : "text-gray-400"
              }`}>
                {profile.x_username || profile.display_name}
              </span>
              {profile.id === currentProfile?.id && (
                <Check className="w-3 h-3 text-neon-indigo" />
              )}
            </button>
          ))}
        </div>
