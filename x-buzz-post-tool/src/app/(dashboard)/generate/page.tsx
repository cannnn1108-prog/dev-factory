...
export default function PostCreatePage() {
  ...
  const getFullText = () => content + (hashtags ? "\n\n" + hashtags : "");
  ...
  const handleSaveDraft = () => {
    ...
    if (savedPosts.length > 50) savedPosts.length = 50;
    localStorage.setItem("buzz_saved_posts", JSON.stringify(savedPosts));
  ...
  const handleSchedule = () => {
    if (!content) return;
    localStorage.setItem("buzz_schedule_draft", getFullText());
        ...
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-neon-indigo/10">
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={!content}
              className="p-2 rounded-lg bg-dark-800 hover:bg-neon-purple/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              title="予約投稿へ"
            >
            ...
            <button
              onClick={handleCopy}
              disabled={!content}
              className="p-2 rounded-lg bg-dark-800 hover:bg-dark-600 transition-all disabled:opacity-50"
              title="コピー"
            >
          ...
          <div className="flex gap-3">
            <button
              ...
              disabled={!content}
              className="flex items-center gap-2 px-4 py-2 rounded-xl border border-neon-indigo/20 text-sm text-gray-300 hover:bg-dark-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button
              ...
              onClick={async () => {
                ...
                setPostError("");
                try {
                  const fullText = getFullText().slice(0, 280);
